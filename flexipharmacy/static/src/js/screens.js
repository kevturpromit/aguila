    ...
    screens.ActionpadWidget.include({
        renderElement: function() {
            ...
            this.$('.empty-cart').click(function(){
		        ...
		        if(lines.length > 0){
		        	self.gui.show_popup('confirm',{
		                'title': _t('¿Vaciar carrito?'),
		                'body': _t('Se borraran todos los artículos asociados con orden actual'),
		                confirm: function(){
		                	order.empty_cart();
                    	            	...
                    	            	if(is_deliver && (!date || !time || !address || !delivery_user_id)){
                    	                	...
                    	                	if (client && credit > client.remaining_credit_limit){
                    	                		setTimeout(function(){
                    	             	    	...
                    	             	    	setTimeout(function(){
                    		             	    	self.gui.show_popup('confirm',{
                    		    		                'title': _t('Orden de venta'),
														'body': _t('¿Desea trasladar la orden a caja?'),
                    		    		                confirm: function(){
	        ...
	        if (product.product_variant_count == 1){
	            ...
	            if(!cached){
	                ...
	                var product_html = QWeb.render('Product',{ 
	                        widget:  this, 
							product: product, 
							product_brand: product.product_brand_id[1],
...
//            //brand Search
		...
		perform_search: function(category, query, buy_result){
			var products;
			if(query){
				products = this.pos.db.search_product_name_description(category.id,query);
    	...
    	start: function(){
	        ...
	        $('div#draft_order').click(function(){
	        	if(self.pos.get_order().get_sale_mode()){
					var order = self.pos.get_order();
					var currentOrderLines = order.get_orderlines();
					var conversion_found  = false;
					_.each(currentOrderLines,function(item) {
						if(item.product.type != 'service'){
							var product_name = item.product.display_name;
							var product_qty = item.get_quantity();
							var qty_available = item.product.qty_available;
							if (product_qty > qty_available){
								conversion_found = true;
								var options = {
									'product': item.product,
									'stock_location_id':self.pos.config.stock_location_id[0],
									'current_order_qty': product_qty,
									'item': item
								};
...
//                });
    ...
    screens.ReceiptScreenWidget.include({
					...
					rpc.query({
						...
						if (orders.length > 0 && orders[0]['invoice_id'] && orders[0]['invoice_id'][1]) {
							...
							rpc.query({
								...
								var dte_number = invoice_number[0]['dte_number']
                                ...
                                var invoice_days = invoice_date.getDate();
                                if (invoice_months<10){
                                    invoice_months = '0'+invoice_months;
                                }
                                if (invoice_days<10){
                                    invoice_days = '0'+invoice_days;
                                }
                                var invoice_date_string =  invoice_date.getFullYear() + '-' + invoice_months + '-' + invoice_days
								self.pos.get_order()['serie'] = serie;
								self.pos.get_order()['dte_number'] = dte_number;
								self.pos.get_order()['dte_date'] = invoice_date_string;
								self.pos.get_order()['uuid'] = uuid;
								self.pos.get_order()['date_invoice'] = date_invoice;
        ...
        print_xml: function() {
			...
			console.log('PRINT XML -----', env, order.is_reprint);
            if(order.get_free_data()){
						...
						if (orders.length > 0) {
							if (orders[0]['invoice_id']) {
								...
								rpc.query({
									...
									self.receipt_data['order']['dte_date'] = invoice_date_string;
									self.receipt_data['order']['uuid'] = uuid;
									self.receipt_data['order']['date_invoice'] = date_invoice;

									console.log('lines', orders[0]['lines'])
									console.log('Order Data', orders[0]);

									self.receipt_data['receipt']['total_with_tax'] = orders[0]['amount_total'];
									self.receipt_data['receipt']['total_paid_amt'] = orders[0]['amount_paid'];
									self.receipt_data['receipt']['amount_due'] = orders[0]['amount_due'];
									
									var line_ids = [];
									_.each(orders[0]['lines'], function(res) {
										line_ids.push(res);
									});
									var lines_data = self.get_orderlines_from_order(line_ids);
									console.log('lines data', lines_data);

									var line_display_data = [];
									_.each(lines_data, function(res) {
										var wrapped_name = self.generate_wrapped_product_name(res.display_name.replace(/ *\[[^\]]*]/, '').trimStart());
										console.log('WWEE', res, res.display_name.replace(/ *\[[^\]]*]/, '').trimStart());
										var order_line = res;
										order_line['product_name_wrapped'] = wrapped_name;
										order_line['discount'] = res.discount;
										order_line['quantity'] = res.qty;
										order_line['unit_name'] = res.uom_id[1];
										order_line['price'] = res.price_unit;
										order_line['price_display'] = res.price_subtotal_incl;
										line_display_data.push(order_line);
									});
									var paymentlines = JSON.parse(orders[0]['pos_xml_receipt_payment_lines']);
									var total_payment = 0;
									_.each(paymentlines.paymentlines, function(res) {
										console.log('Pay line', res);
										total_payment += parseFloat(res.amount);
									});
									self.receipt_data['receipt']['total_paid_amt'] = total_payment;
									self.receipt_data['receipt']['amount_due'] = self.receipt_data['receipt']['total_with_tax'] - total_payment;
									self.receipt_data['receipt']['change'] = total_payment - self.receipt_data['receipt']['total_with_tax'];
									self.receipt_data['order']['paymentlines'] = paymentlines.paymentlines;

									var taxlines = JSON.parse(orders[0]['pos_xml_receipt_taxes_lines']);
									var total_taxes = 0;
									_.each(taxlines.tax_lines, function(res) {
										console.log('Pay line', res);
										total_taxes += parseFloat(res.amount);
									});
									console.log('Taxes 1', taxlines);
									console.log('Taxes 2', taxlines.tax_lines);
									console.log('Taxes 3', total_taxes);
									self.receipt_data['receipt']['tax_details'] = taxlines.tax_lines;
									self.receipt_data['receipt']['total_tax'] = total_taxes;
									
									self.receipt_data['receipt']['orderlines'] = line_display_data;
									console.log('Data OUT', self.receipt_data)
									var receipt = QWeb.render('XmlReceiptReprint', self.receipt_data);
									//console.log('XML', receipt);
									self.pos.proxy.print_receipt(receipt);
									return self.pos.get_order()._printed = true;
								});
							}
						}
					});
				...
				if (orders.length > 0) {
					if (orders[0]['invoice_id']) {
						...
						rpc.query({
							...
							var uuid = invoice_number[0]['uuid']
							var date_invoice = invoice_number[0]['date_invoice']
							console.log('FLEXI MOD CALL ---------------- 213123321231321')
							console.log('Order', self.receipt_data)
							var invoice_date = new Date(dte_date);
							invoice_date.setHours(invoice_date.getHours() + 6);
							var invoice_months = invoice_date.getMonth()+1
...
//	        'click #view_lines': 'click_view_lines',
	    ...
	    click_reprint: function(event){
        	...
        	if(result.pos_normal_receipt_html){
				...
				selectedOrder.print_xml_receipt_payment_lines = result.pos_xml_receipt_payment_lines;
				selectedOrder.print_xml_receipt_taxes_lines = result.pos_xml_receipt_taxes_lines;
