...
class pos_order(models.Model):
    ...
    def _process_order(self, order):
        ...
        if 'pos_salesperson_id' in order:
            print('CALLED ALWAYS', order['pos_salesperson_id'])
        if draft_order_id:
            print('Old order', draft_order_id)
            old_order_data = self.env['pos.order'].browse(draft_order_id)
            current_saleperson_id = 0
            if 'pos_salesperson_id' in order:
                current_saleperson_id = order['pos_salesperson_id']
            original_saleperson_id = 0
            if 'pos_salesperson_id' in old_order_data:
                original_saleperson_id = old_order_data['pos_salesperson_id'].id
            if current_saleperson_id != original_saleperson_id:
                order['pos_salesperson_id'] = original_saleperson_id

            print('New Saleperson Id', order['pos_salesperson_id'])
    ...
    def _order_fields(self, ui_order):
        ...
        order_user_id = ui_order['user_id']
        user_data = self.env['res.users'].browse(order_user_id)
        pos_salesperson_id = 0
        if 'pos_salesperson_id' in ui_order:
            pos_salesperson_id = ui_order['pos_salesperson_id']
        if user_data:
            if user_data.user_role == 'vendor':
                pos_salesperson_id = ui_order.get('pos_salesperson_id', False)
            if user_data.user_role == 'cashier':
                pos_salesperson_id = ui_order['pos_salesperson_id']
        ...
        res.update({
            ...
            'pos_salesperson_id': pos_salesperson_id,
            ...
            'pos_xml_receipt_payment_lines': ui_order.get('pos_xml_receipt_payment_lines'),
            'pos_xml_receipt_taxes_lines': ui_order.get('pos_xml_receipt_taxes_lines'),
        })
        return res
    ...
    pos_xml_receipt_payment_lines = fields.Char(string="Pos XML pagos")
    pos_xml_receipt_taxes_lines = fields.Char(string="Pos XML impuestos")
